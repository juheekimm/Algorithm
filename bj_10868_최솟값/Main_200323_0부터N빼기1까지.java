...
public class Main_200323_0부터N빼기1까지 {
	...
	private static class SegTree {
		...
		int create (int now, int left, int right) {
			...
			return data[now] = Math.min(create(next, left, mid), create(next + 1, mid + 1, right));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < M; i++) {
			...
			sb.append(segTree.query(1, 0, N - 1, Integer.parseInt(st.nextToken()) - 1, 
